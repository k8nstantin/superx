...
#
# This script applies ONLY the kernel layer (v2, G1 design of
# 2026-08-06 — see SUPERX_SCHEMA.md). Modules/adapters that need their
# own tables ship their own schemas + service accounts in later
# operator-approved PRs, each with its own deploy step.
#
# v2 substrate is FRESH: point the server at a new path (e.g.
# rocksdb:./db/superx-v2.db). The v1 data at ./db/superx.db is orphaned
# by the reset and is never read by v2 (SUPERX_SCHEMA.md §6).
#
...
echo "  The 'superx_kernel' service account exists (EDITOR role; token 1h,"
echo "  session never expires)."
