...
export function HistorySection({ s, i, range }: { s: StatsSummary | undefined; i: InsightsSummary | undefined; range: string | null }) {
  ...
  return (
    <>
      {days.length > 0 && (
        <Panel title="The work calendar" scope="all" range={range} note="messages by the day they were written — the agent's own clock · UTC days" mb="md">
          <EChart
            height={190}
            option={{
      ...
      <Grid mb="md" gap="md">
        <Grid.Col span={{ base: 12, lg: 7 }}>
          <Panel title="When the agents work" scope="all" range={range} note="hour of day × day of week · UTC" h="100%">
            <EChart
              height={240}
              option={{
                tooltip: {
