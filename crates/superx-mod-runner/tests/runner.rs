...
// this file. Removed rather than rewritten: testing the runner against
// another module's internals is what coupled them, and the runner reads
// a contract, not a database. Rebuilding that coverage is runner work.
//
// The `prompt` module below is what the replacement looks like: a
// `Graph` deserialized from contract JSON, no entities crate.
