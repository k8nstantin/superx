...
//! `cursor_type` tables.
//!
//! A cursor records "how far has X been processed": the capture
//! engine checkpoints each telemetry source's read position after
//! every poll tick; future workload runners checkpoint resume points
//! the same way. Append-only like everything else —
