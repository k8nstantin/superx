//! Parameter verbs — the substrate plumbing for the no-hardcoded-policy
//! mandate (skill §9).
//!
//! Each parameter is its own `type_definition` row with
...
//!
//! Parameters are attributes **of an entity**. Module- and
//! adapter-scoped parameters target that module's own registry entity
//! (the [`RecordId`] returned by
//! [`Kernel::register_module`](crate::Kernel::register_module)).
//! Global kernel parameters conventionally live on the kernel's own
//! registry entity — [`crate::boot::boot`] registers the kernel itself
//! under the name `"kernel"` and global knobs hang off that entity.
//! The kernel does not special-case a singleton; the convention is
//! boot's to uphold.
