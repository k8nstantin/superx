...
| ~~G8~~ | ~~**`superx-mod-entities`** (2026-08-28, epic #353)~~ — **removed 2026-08-28**: deleted at the operator's instruction. The rebuild shipped a schema that defined its tables `IF NOT EXISTS`, which does not amend a table that is already there, so an instance that had run the earlier shape kept it and refused every create. The module is out of the tree; whatever replaces it starts from an empty directory | crate deleted; rebuild not started |
