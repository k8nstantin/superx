...
| **R2** | **v1.1.0 released** (2026-08-19): the module-framework era — framework + facilities (epic #141), entities graph (epic #166), runner (epic #189), unified session-grouped live dashboard with token telemetry + validated palette (#172–#208), upgrade path (#158), module ledger (v2.2), module contract cemented with the kernel-immutability CI gate (#210) | Tagged from `main`; live OS restarted onto the release build |
| ~~G9~~ | ~~**Runner module** (2026-08-19, epic #189)~~ — **removed 2026-08-28 (#353)**: it reached into the entities module's internals, which D17/D24 forbid, and was deleted rather than patched around. A scheduler that reads the substrate is separate work | crate deleted; rebuild not started |
| **G8** | **`superx-mod-entities` — rebuilt from scratch** (2026-08-28, epic #353; supersedes the epic #166 build). Types, slots, endpoint rules and the note store are gone. THREE TABLES: `entity` is a uuid7 and nothing else, `entity_attribute` is everything said about it including its name, `entity_edge` is a native relation. A label is an entity, and what it MEANS is an attribute on it — so a new label or a new kind of connection is data, never a migration. The module never interprets: a test pins that a label called `role` behaves exactly like one called `zzz`. No CLI — the database is the interface, and its own dashboard runs on its own port | the operator's DBA and Teams-watcher examples build end to end; 8,900 lines became 2,900 |
