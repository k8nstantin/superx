...
| **G8** | **`superx-mod-entities` — third attempt** (2026-08-28, epic #353). THREE TABLES: `entity` is a uuid7 and nothing else, `entity_attribute` is everything said about it including its name, `entity_edge` is a native relation. A label is an entity, so a new label or a new kind of connection is data, never a migration; the module never interprets. **Provisioning removes the DATABASE and rebuilds it**, and a test reads the schema back out of the engine and diffs it against the file — the second attempt defined its tables `IF NOT EXISTS`, left an older shape underneath, and nothing checked | schema verified against the engine; the whole UI flow driven in a browser |
