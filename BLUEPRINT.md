...
| **G9** | **Runner module** (epic #189, R1 in progress): `superx-mod-runner` — schedules entities (D23 two-table SCD-2 intent/fact chains), follows the graph (D25), executes task nodes with agents (D26), in-flight steerable (D27); spec is authoritative in the epic | Built phase-by-phase from issues #190–#194 |
